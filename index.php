<?php
/*
 * Faucet in a BOX - Shortlink and features by coinbox.club
 * https://faucetinabox.com/ - http://coinbox.club
 *
...
require_once("coinbox/coinbox.php");
...
if (isset($_GET['k'])) {
    if (strlen($_GET['k']) == 15) {
        $ip = ip2long(getIP());
        $address = $sql->prepare("SELECT address FROM link WHERE `secret_key` = ? AND `ip` = ?");
        $address->execute(array(trim($_GET['k']), $ip));
        $address = $address->fetch(PDO::FETCH_ASSOC)['address'];
        if ($address) { 
        # Rand amount
            ...
            if ($ret['success']) {
                $q = $sql->prepare("DELETE FROM link WHERE secret_key = ?");
                $q->execute(array(trim($_GET['k'])));
...
} elseif (array_key_exists('address', $_POST) && $data['enabled'] && $data['eligible']) {
    ...
    if ($error) {
        ...
        if ((isset($_POST['shortlink']) and $link_status == 'on') or ($link_force == 'on')) {
            $key = get_token(15);
            $ip = ip2long(getIP());
            $q = $sql->prepare("INSERT INTO link (address, secret_key, ip) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `secret_key` = '$key'");
            $q->execute(array(trim($_POST['address']), $key, $ip));
            for ($i=1; $i <= count($link) ; $i++) { 
                if (!isset($_COOKIE[$i])) {
                    setcookie($i, 'Hi I am Tung', time() + 86400);
                    $url = $link[$i];
